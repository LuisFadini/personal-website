...
function createIcon(iconName) {
	...
	if (!prefix || !name) {
		return h('span', { class: 'filename-icon' });
	}

	const src = new URL(`https://api.iconify.design/${prefix}/${name}.svg`);
	src.searchParams.set('width', '28');
	src.searchParams.set('height', '28');
...
function getFilenameIcon(filename) {
...
function createFilenameTransformer(meta) {
	return {
		...
		pre(node) {
			...
			if (!filename) return;
			const iconName = getFilenameIcon(filename);
			node.children.unshift(h('span.filename', [createIcon(iconName), h('p', filename)]));
		}
	};
}
...
const highlighter = await createHighlighter({
	...
	langs: [
		...
		'properties',
		{
			...
			aliases: ['gitignore'],
			patterns: [{ match: '^#.*', name: 'comment.line.number-sign.ignore' }]
		}
	]
});
		...
		highlighter: async (code, lang = 'text', meta) => {
			const html = escapeSvelte(
				highlighter.codeToHtml(code, {
					...
					theme: 'dracula',
					transformers: [
						createFilenameTransformer(meta),
						{
							pre(node) {
								this.addClassToHast(node, 'not-prose');
								if (meta?.includes('line-numbers')) this.addClassToHast(node, 'line-numbers');
							}
						},
						transformerRemoveNotationEscape(),
